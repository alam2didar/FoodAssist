...
class WorkerEvaluator(QObject):
    ...
    expert_amount_dict = {
        'step 1 gesture 1': 6, 'step 1 gesture 2': 12, 'step 1 gesture 3': 2,
        'step 2 gesture 1': 6, 'step 2 gesture 2': 2, 'step 2 gesture 3': 12,
        'step 3 gesture 1': 6, 'step 3 gesture 2': 14, 'step 3 gesture 3': 0,
        'step 4 gesture 1': 4, 'step 4 gesture 2': 16, 'step 4 gesture 3': 0
    }

    expert_ratio_dict = {
        'step 1 gesture 1': 0.3, 'step 1 gesture 2': 0.6, 'step 1 gesture 3': 0.1,
        'step 2 gesture 1': 0.3, 'step 2 gesture 2': 0.1, 'step 2 gesture 3': 0.6,
        'step 3 gesture 1': 0.3, 'step 3 gesture 2': 0.7, 'step 3 gesture 3': 0,
        'step 4 gesture 1': 0.2, 'step 4 gesture 2': 0.8, 'step 4 gesture 3': 0
    }
    ...
    def evaluate(self, archive_file_name, evaluation_flag):
        success_flag = False
        success_flag_dict = {'step 1': False, 'step 2': False, 'step 3': False, 'step 4': False}
        difference_dict ={'step 1': [-1, -1, -1], 'step 2': [-1, -1, -1], 'step 3': [-1, -1, -1], 'step 4': [-1, -1, -1]}
        # qualitative_result = False
        # qualitative_result_dict = {'step 1': False, 'step 2': False, 'step 3': False, 'step 4': False}
        # troubled_steps = [-1, -1, -1]
        score_dict = {'step 1': 0, 'step 2': 0, 'step 3': 0, 'step 4': 0}
        ...
        if evaluation_flag:
            if archive_file_name:
                ...
                df_step = None
                for index_no in range(1,5):
                    df_step[index_no] = df[df['step'] == f'step {index_no}']
                # df_step_1 = df[df['step'] == 'step 1']
                # df_step_2 = df[df['step'] == 'step 2']
                # df_step_3 = df[df['step'] == 'step 3']
                # df_step_4 = df[df['step'] == 'step 4']
                # perform the same data processing on each step dataset
                for index_no in range(1,5):
                    success_flag_dict[f'step {index_no}'], difference_dict[f'step {index_no}'], score_dict[f'step {index_no}'] = self.process_data_frame(df_step[index_no], index_no)
                # success_flag_dict['step 1'], qualitative_result_dict['step 1'], score_dict['step 1'] = self.process_data_frame(df_step_1, 1)
                # success_flag_dict['step 2'], qualitative_result_dict['step 2'], score_dict['step 2'] = self.process_data_frame(df_step_2, 2)
                # success_flag_dict['step 3'], qualitative_result_dict['step 3'], score_dict['step 3'] = self.process_data_frame(df_step_3, 3)
                # success_flag_dict['step 4'], qualitative_result_dict['step 4'], score_dict['step 4'] = self.process_data_frame(df_step_4, 4)
                # aggregate success_flag from each step
                success_flag = success_flag_dict['step 1'] or success_flag_dict['step 2'] or success_flag_dict['step 3'] or success_flag_dict['step 4']
                ...
                score_percent = int(100*(score_dict['step 1'] + score_dict['step 2'] + score_dict['step 3'] + score_dict['step 4'])/4)
            else:
    ...
    def process_data_frame(self, data_frame_step, step_number):
        ...
        if not df_position.empty:
            ...
            if sum != 0:
                for index in range(3):
                    amount_difference[index] = df_position_amount[index] - self.expert_amount_dict[f'step {step_number} gesture {index+1}']
