    ...
    def evaluate(self, archive_file_name, evaluation_flag):
        if evaluation_flag:
            if archive_file_name is None:
                ...
                df = pd.read_csv(self.archive_file_name, header=None, names=column_names)
                df = df.dropna()
                ...
                if result_ratio >= 0.6 and result_ratio <= 1:
                    ...
                    self.result_text = "In step 1, you have been scoring the vertical gesture (picture 2) more frequently than the expert."
                elif result_ratio >= 0 and result_ratio <= 0.4:
                    self.result_text = "In step 1, you have been scoring the peeling gesture (picture 3) more frequently than the expert."
                # emit result
                self.evaluation_finished.emit(self.fig_1_name, self.fig_2_name, self.result_text)
